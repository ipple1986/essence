    ...
    public static boolean verifyMemberAccess(Class<?> var0, Class<?> var1, Object var2, int var3) {
        ...
        if (var0 == var1) {
            ...
            if (!Modifier.isPublic(getClassAccessFlags(var1))) {
                ...
                var4 = true;
                if (!var5) {//非public 不同包 false
                    return false;
                }
            }

            if (Modifier.isPublic(var3)) {//var1是公开类型的，返回true
                return true;
            } else {
                boolean var6 = false;
                if (Modifier.isProtected(var3) && isSubclassOf(var0, var1)) {//protected, var0为var1子类，可以访问
                    var6 = true;
                }
                //friendly情况
                ...
                if (!var6) {
                    ...
                    if (Modifier.isProtected(var3)) {
                        ...
                        if (var7 != var0) {
                            ...
                            if (!var5 && !isSubclassOf(var7, var0)) {//protected 不同包，无父子关系，不可访问
                                return false;
                            }
                        }
                    }
