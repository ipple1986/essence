...
public class ArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    private void enqueue(E x) {
        ...
        items[putIndex] = x;
        if (++putIndex == items.length)//队尾满了，排到队头
            putIndex = 0;
        count++;// 计数+1
        notEmpty.signal();// 通知 在等待 的队头
    }
    ...
    private E dequeue() {
        ...
        if (++takeIndex == items.length)
            takeIndex = 0;
        count--;
        if (itrs != null)//出队列时，特殊处理一下迭代器，如果存在的话
