 ...
 */
    ...
    private void doReleaseShared() {
        ...
        for (;;) {//防止操作时，有新加点加入，所以循环
            ...
            if (h != null && h != tail) {
                int ws = h.waitStatus;
                //此时说明共享节点已经在head后面，但由于tryAcquireShared条件不满足，shouldParkAfterFailedAcquire
                //将队头状态更新成SIGNAL
                if (ws == Node.SIGNAL) {
                ...
                //设置队头节点的waitStatus=PROPAGATE
                //这种情况是共享锁的第一个到来的线程还没加入到等待队列，此时只初始化了队头head
                else if (ws == 0 &&
                         !compareAndSetWaitStatus(h, 0, Node.PROPAGATE))//PROPAGATE一个短暂的状态
                    continue;                // CAS失败重试,可能第一个等待结点已经加入，重新获取头节点，重试
            }
 ...
 */
    ...
    private void cancelAcquire(Node node) {
        ...
        if (node == tail && compareAndSetTail(node, pred)) {//如果是node是队尾，直接队尾为前一个节点
            ...
            } else {//此处 说明等待队列没有可等待运行的线程，唤醒下一个节点
     ...
     * javaapi.java.util.concurrent.locks.ReentrantReadWriteLock.
     */
    //判断 第一个等待节点是否排它线程
     ...
     * avoid <a href="javaapi.java.util.concurrent.locks.AbstractQueuedSynchronizer#barging">barging</a>.
...
/*
ConditionObject
signal/signalAll(确保当前线程占有排它锁) -> doSignal/doSignalAll(从条件队列将节点加入等待队列，设置waitStatus=0,nextWaiter=null)
await/awaitUninterruptibly/awaitUntil/awaitNanos -> acquireQueue -> tryAcquire

AbstractQueuedSynchronizer

release(int arg) -> tryRelease(int arg) -> unparkSuccessor(Node head)//排它释放
releaseShared(int arg) -> tryReleaseShared(int arg) -> doReleaseShared()//共享释放

void acquire(arg) - > tryAcquire(arg)=false -> acquireQueued(node,arg)//无中断（拦截不抛）排它获取
    boolean acquireQueued(final Node node, int arg) -> tryAcquire(arg)
void acquireShared(int arg) -> tryAcquireShared(arg) < 0 -> doAcquireShared(arg)//无中断共享获取
    void doAcquireShared(int arg) -> tryAcquireShared(arg);

void acquireInterruptibly(int arg) -> tryAcquire(arg)=false -> doAcquireInterruptibly(arg)//有中断排它获取
    void doAcquireInterruptibly(int arg) -> tryAcquire(arg)
void acquireSharedInterruptibly(int arg) -> tryAcquireShared(int arg)<0 -> doAcquireSharedInterruptibly(int arg)//有中断共享获取
    void doAcquireSharedInterruptibly(int arg) -> tryAcquireShared(arg)

boolean tryAcquireNanos(int arg, long nanosTimeout) > tryAcquire(int arg)==true 或者 doAcquireNanos(int arg, long nanosTimeout)
    boolean doAcquireNanos(int arg, long nanosTimeout) -> tryAcquire(arg)
boolean tryAcquireSharedNanos(int arg, long nanosTimeout) -> tryAcquireShared(int arg)>0 或者 (执行）doAcquireSharedNanos(int arg, long nanosTimeout)
    boolean doAcquireSharedNanos(int arg, long nanosTimeout) -> tryAcquireShared(arg)
